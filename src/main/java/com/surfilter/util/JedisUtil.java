...
public class JedisUtil {
			...
			try {
				if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)){
                    try {
                        ...
                        config.setMaxWaitMillis(300000);		// 获取连接时的最大等待毫秒数(如果设置为阻塞时BlockWhenExhausted),如果超时就抛异常, 小于零:阻塞不确定的时间,  默认-1
    ...
    public static void  pip(Map<String,String> map,String type){
        ShardedJedis instance = JedisUtil.getInstance();
        ShardedJedisPipeline pipelined = instance.pipelined();
        long start = System.currentTimeMillis();
        pipelined.hmset(type,map);
    ...
    public static ConcurrentHashMap<String,String> getpip(String type) {
        ShardedJedis instance = JedisUtil.getInstance();
        ShardedJedisPipeline pipelined = instance.pipelined();
        Response<Map<String, String>> mobile = pipelined.hgetAll(type);
        pipelined.sync();
        instance.close();
        return new ConcurrentHashMap(mobile.get());
    }


    //src/redis-cli -h 172.30.154.123 -p 6379 -a 123456 keys "username*" | xargs src/redis-cli -h 172.30.154.123 -p 6379 -a 123456 del
...
//             pipelined.set("username"+i,"testvalueheqitadongdong"+i);
            map.put("username"+i,"testvalueheqitadongdong"+i);
            pipelined.hmset("lijia",map);
            map.clear();
        }
...
//        ConcurrentMap getpip = JedisUtil.getpip();
//        JedisUtil.test();
        JedisUtil.getalldata();
    }


    public static void getalldata(){
        Jedis jedis = new Jedis("172.30.154.123",6379,200);
        jedis.auth("123456");
        Pipeline pipelined = jedis.pipelined();
        long start = System.currentTimeMillis();
        Response<Map<String, String>> lijia = pipelined.hgetAll("mobile");
        pipelined.sync();
        Map<String, String> map = lijia.get();
        long end = System.currentTimeMillis();
        System.out.println(end-start);
        System.out.println(map.size());
    }
